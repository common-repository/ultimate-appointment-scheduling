...
jQuery( document ).ready( function( $ ) {
	...
	jQuery( '.ewd-uasp-about-us-tab-menu-item' ).on( 'click', function() {

		jQuery( '.ewd-uasp-about-us-tab-menu-item' ).removeClass( 'ewd-uasp-tab-selected' );
		jQuery( '.ewd-uasp-about-us-tab' ).addClass( 'ewd-uasp-hidden' );

		var tab = jQuery( this ).data( 'tab' );

		jQuery( this ).addClass( 'ewd-uasp-tab-selected' );
		jQuery( '.ewd-uasp-about-us-tab[data-tab="' + tab + '"]' ).removeClass( 'ewd-uasp-hidden' );
	} );

	jQuery( '.ewd-uasp-about-us-send-feature-suggestion' ).on( 'click', function() {

		var feature_suggestion = jQuery( '.ewd-uasp-about-us-feature-suggestion textarea' ).val();
		var email_address = jQuery( '.ewd-uasp-about-us-feature-suggestion input[name="feature_suggestion_email_address"]' ).val();
	
		var params = {};

		params.nonce  				= ewd_uasp_admin_php_data.nonce;
		params.action 				= 'ewd_uasp_send_feature_suggestion';
		...
		jQuery( '.ewd-uasp-about-us-feature-suggestion' ).prepend( '<p>Thank you, your feature suggestion has been submitted.' );
	} );
} );
...
jQuery( document ).ready( function() {
	...
	if ( ! jQuery( '.nav-tab[title="Emails"]' ).hasClass( 'nav-tab-active' ) ) {
		jQuery( '.ewd-uasp-settings-preview' ).prevAll( 'h2' ).hide();
		jQuery( '.ewd-uasp-settings-preview' ).prevAll( '.sap-tutorial-toggle' ).hide();
		jQuery( '.ewd-uasp-settings-preview .sap-tutorial-toggle' ).hide();
	}
});
