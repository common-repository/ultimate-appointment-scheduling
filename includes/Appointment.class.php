...
class ewduaspAppointment {
	...
	// Information on the loaded appointment, from database
	public $id = 0;
	...
	public $paypal_prepaid;
	public $paypal_receipt;
	public $wc_prepaid;
	public $wc_order_id;
	...
	// Stores any validation error when a booking is submitted
	...
	public function load_appointment( $db_appointment ) {
		...
		$this->reminder_sent	= ! empty( $db_appointment->Appointment_Reminder_Email_Sent ) ? explode( ',', $db_appointment->Appointment_Reminder_Email_Sent ) : array();
	...
	public function validate_submission() {
		...
		if ( $ewd_uasp_controller->settings->get_setting( 'use-captcha' ) ) {
			...
			if ( $ewd_uasp_controller->settings->get_setting( 'captcha-type' ) == 'recaptcha' ) {
				...
				if ( ! isset( $_POST['g-recaptcha-response'] ) ) {
					...
					$this->validation_errors[] = array(
						...
						'message'	=> __( 'Please fill out the reCAPTCHA box  before submitting.', 'ultimate-appointment-scheduling' ),
					);

				}
				else {

					$secret_key = $ewd_uasp_controller->settings->get_setting( 'captcha-secret-key' );
					...
					if ( json_last_error() != JSON_ERROR_NONE ) {
						...
						if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
							...
							error_log( 'UASP reCAPTCHA error. Raw respose: ' . print_r( [$json_response], true ) );
						}
					}
	
					if ( ! $response->success ) {

						$message = __( 'Please fill out the reCAPTCHA box again and re-submit.', 'ultimate-appointment-scheduling' );
						
						if ( $reCaptcha_error ) {

							$message .= __( 'If you encounter reCAPTCHA error multiple times, please contact us.', 'ultimate-appointment-scheduling' );
						}
