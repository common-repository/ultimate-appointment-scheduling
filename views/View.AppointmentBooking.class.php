...
class ewduaspViewAppointmentBooking extends ewduaspView {
	...
	public $appointment_selection;
	public $display_type;
	public $redirect_page;
	public $update_message;
	public $cancellation_success_message;
	public $cancellation_failure_message;
	public $confirmation_success_message;
	public $confirmation_failure_message;

	// Which page to display (login, edit, appointment)
	public $display;

	// Whether a calendar or dropdown should be used for appointment selection
	public $calendar_mode;

	// Stored from settings
	public $multi_step_booking;
	public $paypal_prepayment;
	public $min_date;
	public $max_date;
	...
	// Flag for if an appointment request has been submitted
	public $booking_submitted;

	// Edit appointment details, if set
	public $edit_appointment_search_results;
	...
	public function maybe_print_captcha() {
		...
		if ( ! $ewd_uasp_controller->settings->get_setting( 'use-captcha' ) ) { return; }
		
		if ( $ewd_uasp_controller->settings->get_setting( 'captcha-type' ) == 'recaptcha' ) { $template = $this->find_template( 'recaptcha' ); }
		else { $template = $this->find_template( 'captcha' ); }
	...
	public function set_display_parameters() {
		...
		elseif ( empty( $this->failure ) and ! empty( $_REQUEST['ewd_uasp_payment_submit'] ) ) { $this->display = 'payment'; }
	...
	public function enqueue_assets() {
		...
		$calendar_offset = ! empty( $calendar_offset ) ? $calendar_offset : '0_offsetDay';
		...
		if ( $this->calendar_mode ) {
			...
			if ( empty( $ewd_uasp_controller->settings->get_setting( 'use-captcha' ) ) )  { return; }

			if ( $ewd_uasp_controller->settings->get_setting( 'captcha-type' ) != 'recaptcha' ) { return; }
	
			wp_enqueue_script( 'ewd-uasp-google-recaptcha', 'https://www.google.com/recaptcha/api.js?hl=' . get_locale() . '&render=explicit&onload=ewduaspLoadRecaptcha' );
			wp_enqueue_script( 'ewd-uasp-process-recaptcha', EWD_UASP_PLUGIN_URL . '/assets/js/ewd-uasp-recaptcha.js', array( 'ewd-uasp-google-recaptcha' ) );
	
			$args = array(
				'site_key'	=> $ewd_uasp_controller->settings->get_setting( 'captcha-site-key' ),
			);
	
			wp_localize_script( 'ewd-uasp-process-recaptcha', 'ewd_uasp_recaptcha', $args );
		}
