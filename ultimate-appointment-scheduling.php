...
class ewduaspInit {
	...
	protected function includes() {
		...
		require_once( EWD_UASP_PLUGIN_DIR . '/includes/AboutUs.class.php' );
	...
	function register_assets() {
		...
		wp_register_script( 'full-calendar', EWD_UASP_PLUGIN_URL . '/assets/js/fullcalendar.js', array( 'jquery', 'moment' ), EWD_UASP_VERSION, true );
	...
	public function plugin_action_links( $links, $plugin ) {
		...
		if ( $plugin == EWD_UASP_PLUGIN_FNAME ) {
			...
			if ( ! $ewd_uasp_controller->permissions->check_permission( 'premium' ) ) {

				array_unshift( $links, '<a class="ewd-uasp-plugin-page-upgrade-link" href="https://www.etoilewebdesign.com/license-payment/?Selected=UASP&Quantity=1&utm_source=wp_admin_plugins_page" title="' . __( 'Try Premium', 'ultimate-appointment-scheduling' ) . '" target="_blank">' . __( 'Try Premium', 'ultimate-appointment-scheduling' ) . '</a>' );
			}
		...
		if ( ! $ewd_uasp_controller->permissions->check_permission( 'styling' ) or get_option( 'EWD_UASP_Trial_Happening' ) == 'Yes' ) {
			...
			<div class="ewd-uasp-dashboard-new-upgrade-banner">
				...
				<div class="ewd-uasp-dashboard-banner-buttons">
					<a class="ewd-uasp-dashboard-new-upgrade-button" href="https://www.etoilewebdesign.com/license-payment/?Selected=UASP&Quantity=1&utm_source=uasp_admin&utm_content=banner" target="_blank">UPGRADE NOW</a>
				</div>
